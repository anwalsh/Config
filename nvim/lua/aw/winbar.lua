local folder_icon = require("aw.settings").icons.kinds.Folder
...
function M.render()
    ...
    if vim.api.nvim_win_get_width(0) < math.floor(vim.o.columns / 3) then
        ...
        local special_dirs = {
            ...
            DOTFILES = g.dotfiles,
            HOME = env.HOME,
            PERSONAL = g.projects_dir .. "/github.com/anwalsh",
        }
    ...
    return table.concat({
        ...
        prefix,
        table.concat(
            vim.iter.map(function(segment) return string.format("%%#Winbar#%s", segment) end, vim.split(path, "/")),
            separator
        ),
    })
end
...
vim.api.nvim_create_autocmd("BufWinEnter", {
    group = vim.api.nvim_create_augroup("aw/winbar", { clear = true }),
    desc = "Attach winbar",
